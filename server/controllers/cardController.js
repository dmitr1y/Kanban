...
exports.get = (req, res) => {
  Dashboard.findById(req.query.dashboardId, (err, dashboard) => {
    if (err || dashboard == null) {
      res.status(404).json({
        message: 'Dashboard not found.',
      });
    } else {
      let column = dashboard.columns.id(req.query.columnId);

      if (column) {
        let card = dashboard.columns.id(req.query.columnId)
            .cards
            .id(req.query.cardId);
...
exports.update = (req, res) => {
  if (!req.body.name || !req.body.columnId || !req.body.position ||
      !req.body.dashboardId || !req.body.cardId || !req.body.description ||
      !req.body.isUrgently) {
    return res.status(404).json({
      message: 'Not found',
    });
